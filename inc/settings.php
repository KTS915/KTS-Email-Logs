...
function kts_email_logs_storage_render() {
	...
	if ( empty( $email_logs ) || empty( $email_logs['storage'] ) || ! in_array( $email_logs['storage'], $storage_options ) ) {
		$email_logs['storage'] = 604800;
	} ?>
	...
	<fieldset>
		<label>
			<input type="radio" name="email-logs[storage]" value="1" <?php checked( in_array( $email_logs['storage'], $storage_options ) ); ?>>
			<span class="date-time-text date-time-custom-text"><?php _e( 'Yes: delete messages that are over', 'kts_email_logs' ); ?><span>
				...
				<select name="timescale">
					...
					<option value="604800" <?php selected( $email_logs['storage'], 604800 ); ?>><?php _e( '1 week', 'kts_email_logs' ); ?></option>

					<option value="1209600" <?php selected( $email_logs['storage'], 1209600 ); ?>><?php _e( '2 weeks', 'kts_email_logs' ); ?></option>

					<option value="1814400" <?php selected( $email_logs['storage'], 1814400 ); ?>><?php _e( '3 weeks', 'kts_email_logs' ); ?></option>

					<option value="2419200" <?php selected( $email_logs['storage'], 2419200 ); ?>><?php _e( '4 weeks', 'kts_email_logs' ); ?></option>

					<option value="15780000" <?php selected( $email_logs['storage'], 15780000 ); ?>><?php _e( '6 months', 'kts_email_logs' ); ?></option>
